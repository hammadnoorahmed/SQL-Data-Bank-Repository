...
#just to check date data for any discrepecny 
select txn_date from customer_transactions 
where txn_date like '%9999%';

#average days per node of customer
select round(Avg(DATEDIFF(end_date,start_date)),1) as Average_days_nodes
from customer_nodes
where end_date not like '%9999%';

#medion, 80thpercentile and 95thpercentile of all the regions
with rows_ as (
...
#deposit account and transactions
With total_deposits as
(
Select customer_id, count(txn_type) as Total_deposit, sum(txn_amount) as total_transaction_amount
from customer_transactions
where txn_type = 'deposit'
group by customer_id)
Select round(avg(Total_deposit),0) as avg_total_deposit,
round(avg(total_transaction_amount),1) as avg_deposit_amount
from total_deposits;


#monthly data for more than one deposit
...
-- finding average using three CTEs
...
#customer update with each transaction
Select customer_id, txn_date, month(txn_date) as month_, sum(case
when txn_type = 'deposit' then txn_amount else 0 - txn_amount end) 
over (partition by customer_id order by customer_id ASC, txn_date ASC) as CustomerBalance 
from customer_transactions;

# this can be a proper statement indicating all transactions for customers using SQL
#using customer_id we can extraxt info for single customer as well 
